...
export class AuthStore {
    ...
    login = async (params: IAuthStore.LoginParams) => {
        const { result }: any = await request.post<IAuthStore.UserInfo>('/system/user/login', params)
        this.setUserInfo(isPlainObject(result) ? result : null)
        localStorage.setItem(LOCALSTORAGE_KEYS.USERINFO, JSON.stringify(result))
        history.replace('/')
    }
